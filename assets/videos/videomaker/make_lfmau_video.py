...
A paper video for "LFMAU-Net: Multi Attention U-Net with Adaptive Lesion
Focal Loss for Breast Ultrasound Segmentation" (ICCIT 2025), built straight
from the paper PDF. Figures are cropped from the source at high DPI, so
nothing is a screenshot and nothing is redrawn; the tables are typeset by
the script from values checked against the paper's own text layer.

    pip install pymupdf pillow numpy imageio imageio-ffmpeg
    python make_lfmau_video.py "papers/lfmau.pdf"

Default is a short loop: the architecture, the qualitative comparison, the
benchmark. --full adds the CBAM detail, the ablation, and title and closing
cards.

    --grid     one PNG per page with a labelled coordinate overlay
    --themes   WCAG contrast table for the chosen palette
    --palette  pastel family; give each paper its own so two clips on one
               page never read as the same deck
"""
...
TABLES = {
    "ablation": dict(
        title="Ablation: what each component contributes",
        cols=[("Configuration", "l", False), ("Dice", "r", True),
              ("IoU", "r", True), ("Recall", "r", True)],
        rows=[["Base U-Net", 74.77, 59.71, 63.38],
              ["Attention U-Net", 75.75, 60.97, 65.57],
              ["LFMAU-Net (Dice + CE)", 80.57, 67.46, 82.11],
              ["LFMAU-Net (ALFL)", 82.25, 69.85, 85.58]],
        ours=3, unit="%",
        note="ALFL adds 1.68 Dice over the same network trained with Dice + cross-entropy"),
    "sota": dict(
        title="Against published baselines on BUSI",
        cols=[("Method", "l", False), ("Dice", "r", True),
              ("IoU", "r", True), ("Accuracy", "r", True)],
        rows=[["U-Net", 75.19, 65.33, 92.45],
              ["U-Net++", 75.03, 61.59, 92.18],
              ["Attention U-Net", 77.21, 64.72, 92.87],
              ["ResU-Net", 75.20, 61.72, 92.51],
              ["TransUNet", 81.11, 69.19, 95.75],
              ["MedT", 76.93, 63.89, 87.35],
              ["LFMAU-Net", 82.25, 69.85, 96.32]],
        ours=6, unit="%", note="TransUNet is the closest at 81.11 Dice"),
}
...
FIGURES = {
    "fig1_arch":   (2, 0.070, 0.055, 0.940, 0.425),
    "fig2_cbam":   (3, 0.060, 0.285, 0.490, 0.452),
    "fig3_visual": (4, 0.070, 0.545, 0.935, 0.860),
}

# Figures whose ground must NOT be repainted. tint_to_bg moves bright
# neutral pixels, and a white segmentation mask is exactly that -- it scores
# 100%, so fig3 would have its masks recoloured, and bright ultrasound
# speckle repainted about half way. Those pixels are the data.
NO_TINT = {"fig3_visual"}

# Stages of fig1, read off a grid overlay of the TRIMMED figure.
#
# Each stage is a LIST of boxes, not one band. The U runs on diagonals --
# encoder, CBAM, gates and decoder all step down-and-right together -- so a
# single axis-aligned band around any one of them swallows its neighbours.
# The first version overlapped 45% between encoder and CBAM and 77% between
# gates and decoder, which made consecutive steps look like the same frame.
PIPELINE_STEPS = [
    [(0.000, 0.00, 0.108, 0.21)],                                   # BUSI input
    [(0.115, 0.00, 0.172, 0.19), (0.152, 0.20, 0.209, 0.39),        # EfficientNet-B4,
     (0.192, 0.39, 0.249, 0.59), (0.229, 0.60, 0.286, 0.80),        # five scales
     (0.332, 0.78, 0.389, 1.00)],
    [(0.192, 0.00, 0.249, 0.19), (0.237, 0.20, 0.294, 0.39),        # CBAM on every
     (0.272, 0.39, 0.329, 0.59), (0.307, 0.60, 0.364, 0.80),        # skip
     (0.422, 0.78, 0.479, 1.00)],
    [(0.680, 0.04, 0.730, 0.16), (0.610, 0.24, 0.660, 0.36),        # attention gates,
     (0.535, 0.44, 0.585, 0.56), (0.450, 0.63, 0.500, 0.75)],       # one per level
    [(0.740, 0.00, 0.805, 0.20), (0.665, 0.20, 0.730, 0.40),        # decoder blocks
     (0.590, 0.40, 0.655, 0.60), (0.515, 0.60, 0.580, 0.80)],
    [(0.810, 0.00, 0.870, 0.20), (0.890, 0.00, 1.000, 0.21)],       # upsample + mask
]

PIPELINE_CLOSE = "82.25% Dice, 69.85% IoU, 85.58% recall on BUSI"

# ---------------------------------------------------------------- beats
BEATS = [
    dict(kind="title", seconds=4.0),
    dict(kind="pipeline", figure="fig1_arch", seconds=17.0,
         head="Attention at both ends of the U",
         sub="EfficientNet-B4 encoder, CBAM on every skip, attention gates in the decoder",
         steps=["A 224x224 B-mode ultrasound frame enters",
                "EfficientNet-B4, ImageNet weights, first 30 layers frozen",
                "CBAM refines each skip: channel first, then spatial",
                "Attention gates weight those skips on decoder context",
                "The decoder upsamples, concatenates and refines",
                "Learned 2x upsample, 1x1 classifier, sigmoid at 0.5"]),
    dict(kind="figure", figure="fig2_cbam", seconds=5.0,
         head="CBAM, in sequence",
         sub="Channel attention from pooled descriptors, then a spatial mask over those channels",
         chips=["reduction r in {8, 16}", "kernels k in {3, 7}"]),
    dict(kind="table", table="ablation", seconds=6.5,
         head="Where the gain comes from",
         sub="Attention and the adaptive loss contribute separately"),
    dict(kind="table", table="sota", seconds=7.5,
         head="Against published baselines",
         sub="Dice, IoU and pixel accuracy on the BUSI dataset"),
    dict(kind="figure", figure="fig3_visual", seconds=6.0,
         head="Sharper boundaries, fewer false positives",
         sub="Input, ground truth, U-Net, Attention U-Net, ours, heatmap and overlay",
         chips=["small lesions recovered", "low-contrast regions held"]),
    dict(kind="end", seconds=5.0,
         lines=["Adaptive Lesion Focal Loss boosts samples where lesions are sparse",
                "Attention on the skips and in the decoder, not just one of the two",
                "82.25% Dice, 69.85% IoU, 85.58% recall on BUSI"]),
]


# The default cut: three beats, no title or end card, because the page
# around it already says whose paper this is. The qualitative panel leads in
# the poster because a row of lesions reads at thumbnail size; the
# architecture and the benchmark explain it.
SHORT = [
    dict(kind="figure", figure="fig3_visual", seconds=3.6,
         head="Sharper boundaries than U-Net and Attention U-Net",
         sub="",
         chips=["82.25% Dice", "69.85% IoU"]),
    dict(kind="table", table="sota", seconds=5.5,
         head="Against published baselines",
         sub="Dice, IoU and pixel accuracy on BUSI"),
    dict(kind="pipeline", figure="fig1_arch", seconds=8.0,
         head="Attention at both ends of the U",
         sub="EfficientNet-B4 encoder, CBAM skips, attention-gated decoder",
         steps=["224x224 ultrasound frame", "EfficientNet-B4 encoder",
                "CBAM on every skip", "Attention gates weight the skips",
                "Decoder upsamples and refines", "Sigmoid lesion mask"]),
]
...
def verify_tables(doc):
    """Every printed value must appear in the paper's text layer. Cheap
    insurance against a transcription slip in TABLES."""
    import re
...
def tint_to_bg(im, bg, cut=228):
    ...
    if key not in _TINT_CACHE:
        ...
        w = (np.clip((mx - cut) / (255.0 - cut), 0, 1) * ((mx - mn) < 30))
...
def frame_table(beat, args, t):
    ...
    spans = {i: (min(r[i] for r in rows), max(r[i] for r in rows))
    ...
    for slot, ri in enumerate(order):
        ...
        for i, (_, align, bar) in enumerate(cols):
            ...
            cw = label_w if i == 0 else num_w
            val = rows[ri][i]
            txt = str(val) if i == 0 else "%.2f%s" % (val, spec["unit"])
...
def main():
    ...
    ap.add_argument("--title", default="LFMAU-Net")
    ap.add_argument("--title-lines", nargs="+",
                    default=["LFMAU-Net",
                             "Multi-attention U-Net with adaptive lesion focal loss"])
    ap.add_argument("--authors", default="Md Kawsar Mahmud Khan Zunayed, Abdullah Al Shafi, "
                                         "Soumit Kumar Kundu, Safin Ahmmed")
    ap.add_argument("--affil", default="Khulna University of Engineering & Technology")
    ap.add_argument("--venue", default="ICCIT 2025")
    ap.add_argument("--source", default="Breast Ultrasound Images (BUSI) dataset")
    ...
    ap.add_argument("--palette", default="sage", choices=list(PALETTES),
    ...
    ap.add_argument("--out", default="lfmau")
    args = ap.parse_args()
