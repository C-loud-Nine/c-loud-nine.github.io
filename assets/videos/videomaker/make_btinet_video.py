#!/usr/bin/env python3
r"""
make_btinet_video.py

An animated graphical abstract for BTI-Net: an annotated walk through the
architecture figure, the two result tables typeset from data, and the
qualitative grid across all three modalities.

    pip install pymupdf pillow numpy imageio imageio-ffmpeg
    python make_btinet_video.py "papers/btinet.pdf"

    --grid   write a coordinate overlay for Figure 1 and exit, so the STAGES
             boxes below can be read off instead of guessed

One cut, not two. Three modalities is the paper's selling point, so the
qualitative grid stays in; what would need its own explanation (the
nineteen supplementary uncertainty figures) stays out.

Type scale, canvas geometry and pacing follow make_dash_abstract.py so the
two clips sit together on the same page.

Every number on screen was checked against the PDF text layer: Table 4 for
the ablation, Table 5 for the comparison. Table 5 reports IoU and Acc per
dataset; the columns taken here are the three IoU ones.
"""
...
# ---------------------------------------------------------------- palette
BG = (240, 244, 246)            # cool slate paper, distinct from DASH's cream
INK = (26, 34, 40)
MUTED = (96, 110, 118)
ACCENT = (19, 102, 114)         # teal
PINE = (16, 58, 70)
SOFT = (208, 220, 226)
CARD = (252, 253, 254)
...
STAGES = [
    ("One image, two tasks",
     [(0.020, 0.000, 0.095, 0.360)]),
    ("EfficientNet-B4, three dilations fused",
     [(0.022, 0.185, 0.345, 0.750)]),
    ("TIM exchanges boundary and semantic cues",
     [(0.380, 0.165, 0.605, 0.770)]),
    ("UPA gates it per sample",
     [(0.608, 0.175, 0.835, 0.710)]),
    ("Refinement accumulates, 14x14 to 112x112",
     [(0.840, 0.075, 0.905, 0.690)]),
    ("One pass, mask and label",
     [(0.912, 0.015, 1.000, 0.970)]),
]
...
FIGURES = {
    "arch":   (3, 0.085, 0.085, 0.925, 0.300),
    "visual": (7, 0.155, 0.080, 0.855, 0.640),
}
...
TABLES = {
    "ablation": dict(
        cols=[("Components", "l", False), ("Seg IoU", "r", True), ("Clf Acc", "r", True)],
        rows=[["Baseline", 67.43, 84.62],
              ["MSCF", 69.95, 88.89],
              ["TIM", 69.20, 86.32],
              ["MSCF + TIM", 72.14, 90.84],
              ["TIM + UPA", 70.48, 91.47],
              ["MSCF + TIM + UPA", 74.50, 93.16]],
        ours=5, dec=2,
        note="Adaptive gating is worth +2.36 IoU over fixed interaction"),
    "joint": dict(
        cols=[("Method", "l", False), ("BUSI", "r", True),
              ("HAM10000", "r", True), ("BRISC", "r", True)],
        rows=[["MTAN (2019)", 68.41, 87.24, 68.84],
              ["MTANet (2024)", 71.24, 87.12, 71.84],
              ["MTL-OCA (2025)", 72.14, 86.84, 71.24],
              ["MTI-Net (2020)", 73.06, 87.84, 73.84],
              ["DenseMTL (2023)", 72.83, 88.14, 74.61],
              ["BTI-Net", 74.50, 89.80, 76.74]],
        ours=5, dec=2,
        note="Segmentation IoU, ahead on all three modalities"),
}
...
def visual_frame(halves, t, args):
    ...
    for hf in halves:
        ...
        scaled.append(im)
    tw_ = sum(i.width for i in scaled) + gap * (len(scaled) - 1) + 4 * pad
    x = (W - tw_) // 2
    fy = 190
    ...
    header(d, S, args, 1.0)
    d.rectangle([72 * S, 152 * S, (72 + 130) * S, 156 * S], fill=fade(ACCENT, 1))
    badge(d, S, "3 modalities", ease_out(seg(t, 0.02, 0.26)))
...
def build(args):
    ...
    segs = [
        ...
        (lambda i: table_frame(TABLES["joint"], "Versus prior MTL", i / LT, args), LT),
        (lambda i: table_frame(TABLES["ablation"], "Ablation", i / LT, args), LT),
...
def main():
    ...
    ap.add_argument("pdf")
    ap.add_argument("--title", default="BTI-Net")
    ap.add_argument("--subtitle", default="Bidirectional decoder-level task interaction, "
                                          "gated by uncertainty")
    ap.add_argument("--final-caption",
                    default="One pass, two tasks, refined by each other")
    ap.add_argument("--visual-caption",
                    default="Ultrasound, brain MRI and dermoscopy, against five baselines")
    ap.add_argument("--arch-seconds", type=float, default=14.0)
    ...
    ap.add_argument("--out", default="btinet")
    args = ap.parse_args()
